...
export interface LayerFrame {
  id: string
  elements: SceneElement[]  // Elements at this frame
  frameNumber: number
  length: number           // Number of time slots this frame spans
}
...
export interface Scene {
  ...
  layers: SceneLayer[]
  active: boolean
  order?: number  // Add optional order property for scene ordering
}
...
export interface SceneLayer {
  ...
  locked: boolean
  opacity: number
  elements: SceneElement[]  // Current elements being edited
  frames: LayerFrame[]      // Timeline frames for this layer
  zIndex: number
}
...
export interface SceneStore {
  ...
  // Layer actions
  createLayer: (sceneId: string, name: string, type: SceneLayer['type']) => SceneLayer
  removeLayer: (sceneId: string, layerId: string) => void
  updateLayer: (sceneId: string, layerId: string, updates: Partial<SceneLayer>) => void
  reorderLayers: (sceneId: string, layerIds: string[]) => void
  // Element actions
  addElement: (sceneId: string, layerId: string, element: Omit<SceneElement, 'id'>) => string
  removeElement: (sceneId: string, layerId: string, elementId: string) => void
  updateElement: (sceneId: string, layerId: string, elementId: string, updates: Partial<SceneElement>) => void
  setActiveScene: (sceneId: string) => void
  setSelectedElement: (element: SceneElement | null) => void
  setCurrentFrame: (frame: number) => void
  captureFrame: (sceneId: string) => void  // Capture current canvas state
  deleteFrame: (sceneId: string, frameId: string) => void
  playAnimation: () => void
  pauseAnimation: () => void
  loadFrame: (sceneId: string, frameNumber: number) => void
  clearCanvas: (sceneId: string) => void
  setPlaybackSettings: (fps: number, duration: number) => void
  selectFrame: (sceneId: string, frameNumber: number) => void
  duplicateFrame: (sceneId: string, frameNumber: number) => void
  setOnionSkinning: (settings: Partial<SceneStore['onionSkinning']>) => void
  setLooping: (loop: boolean) => void
  exportVideo: (settings: ExportSettings) => Promise<void>
  setExportProgress: (progress: number) => void
  resizeFrame: (sceneId: string, frameId: string, newLength: number, newFrameNumber?: number) => void
  stopAnimation: () => void
}
...
export const useSceneStore = create<SceneStore>((set, get) => ({
  ...
  reorderLayers: (sceneId, layerIds) => {
    set((state) => {
      ...
      const orderedLayers = layerIds
        ...
        .filter((l): l is SceneLayer => l !== undefined)
        .map((layer, index) => ({
          ...layer,
          zIndex: index
        }))
  ...
  addElement: (sceneId, layerId, element) => {
    ...
    set((state) => {
      ...
      const newFrame: LayerFrame = {
        id: existingFrame?.id || generateId(),
        elements: [
          // Keep existing elements from other layers
          ...(existingFrame?.elements || []).filter((e: SceneElement) => e.layerType !== layer.type),
          // Add elements from the active layer
          ...layer.elements.map((e: SceneElement) => ({ ...e })),
          newElement
        ],
    ...
    set((state) => {
          ...
          ? {
              ...
              layers: s.layers.map(l =>
                l.id === layerId
                  ? {
                      ...l,
                      elements: l.elements.map(element =>
      ...
      const newFrame: LayerFrame = {
        ...
        elements: [
          ...
          ...updatedLayer.elements.map((e: SceneElement) => ({ ...e }))
        ],
        frameNumber: currentFrame,
        length: existingFrame?.length || 1
      }
  ...
  setSelectedElement: (element) => {
    set((state) => {
      ...
      if (element) {
        const sceneWithElement = updatedScenes.find(scene =>
          scene.layers.some(layer =>
            layer.elements.some(e => e.id === element.id)
          )
        );

        if (sceneWithElement) {
          const layerWithElement = sceneWithElement.layers.find(layer =>
            layer.elements.some(e => e.id === element.id)
          );
      ...
      const frame: LayerFrame = {
        ...
        elements: [
          ...
          ...(activeLayer.frames.find(f => f.frameNumber === currentFrame)?.elements || [])
            .filter((e: SceneElement) => e.layerType !== activeLayer.type),
          // Add elements from the active layer
          ...activeLayer.elements.map((e: SceneElement) => ({ ...e }))
        ],
        frameNumber: currentFrame,
  ...
  playAnimation: () => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (deltaTime >= frameTime) {
        ...
        const framesWithContent = activeScene.layers
          .flatMap(layer => 
            layer.frames
              ...
              .map(f => ({
                ...f,
                elements: f.elements.map(e => ({ ...e } as SceneElement))
              }))
          )
          .sort((a, b) => a.frameNumber - b.frameNumber);
        ...
        // Find current frame
        const currentFrameData = framesWithContent.find(f => 
        ...
        if (currentFrameData && currentFrame < currentFrameData.frameNumber + (currentFrameData.length - 1)) {
          ...
          // Find next frame
          const nextFrame = framesWithContent.find(f => 
            (currentFrameData 
  ...
  loadFrame: (sceneId, frameNumber) => {
    set((state) => {
      ...
      if (scene.layers.some(l => l.elements.length > 0)) {
        ...
        const newFrames: LayerFrame[] = existingFrames.map((frame, index) => ({
          id: frame.id,
  ...
  duplicateFrame: (sceneId, frameNumber) => {
    set((state) => {
      ...
      const newFrame: LayerFrame = {
        id: generateId(),
        elements: sourceFrame.elements.map((e: SceneElement) => ({ ...e })),
