...
import type { StateUpdate, Scene } from '../types.js';
...
export function handleStateRequests(socket: Socket, io: any, stateManager: StateManager) {
...
export function transformState(newState: any): StateUpdate {
  ...
  const activeScene = scenes.find((s: Scene) => s.active) || null;
  ...
  const activeLayer = newState.activeLayer || 
    (activeScene?.layers.find((l: { id: string }) => l.id === newState.activeLayer)?.id) ||
